...
using namespace indexer_header;
...
public:
    ...
    indexer(name receiver, name code,  datastream<const char*> ds):contract(receiver, code, ds){}

    ACTION broadcast(string user_id, string doc_type, string data) {
        uint64_t compoundScope = std::hash<string>{}(user_id + ":" + doc_type);

        Json json = Json::parse(data);

        auto contents = Contents(get_self(), compoundScope);
